...
2. 选择一个本地文件。
3. 应用创建一篇与文件同名的 Notion 页面，并把原文件作为附件写入。
4. 不超过 1 MiB 的文本或代码文件会额外生成内容预览。
5. 每次成功或失败都会写入本地上传记录，包括文件路径、大小、SHA-256、时间、状态、Notion 页面地址和错误原因。
6. 不超过 20 MiB 的文件使用单段上传；更大的文件自动使用 10 MiB 分片并在全部发送后完成合并。
...
- Notion 付费工作区的 API bot 最多可上传 5 GiB 文件。
