...
上传正文按 256 KiB 流式发送，并约每 200 ms 刷新一次诊断数据。速度根据 HTTP 请求体被客户端持续消费的字节数计算，适合判断上传链路是否明显偏慢，但不是操作系统网卡层的精确流量计数。显示的 URL 会移除查询参数和片段，避免把未来可能出现的临时签名参数写入界面或传输历史。

传输中心会在完成记录中保留诊断摘要，将总耗时拆分为 SHA/磁盘处理、文件上传/API 等待和 Notion 页面/区块索引写入三部分。平均有效上传速度低于 512 KiB/s 时会提示网络链路可能是主要瓶颈；本地校验或 Notion API 阶段明显更慢时也会分别提示。
