...
impl NotionClient {
    pub fn new(token: String) -> Result<Self> {
        let client = Client::builder().user_agent("notion-file/0.1.0").build().context("无法初始化 HTTP 客户端")?;
        Ok(Self { client, token })
    }

    fn request(&self, method: reqwest::Method, url: String) -> reqwest::RequestBuilder {
        self.client.request(method, url).bearer_auth(&self.token).header("Notion-Version", NOTION_VERSION).header("Accept", "application/json")
    }
    ...
    async fn parse(response: Response) -> Result<Value> {
        ...
        if !status.is_success() {
            let message = serde_json::from_str::<Value>(&text).ok().and_then(|value| value.get("message").and_then(Value::as_str).map(str::to_owned)).unwrap_or(text);
    ...
    pub async fn archive_page(&self, page_id: &str) -> Result<()> {
        let page_id = normalize_page_id(page_id)?;
        let response = self.request(reqwest::Method::PATCH, format!("{NOTION_BASE_URL}/pages/{page_id}")).json(&json!({ "archived": true })).send().await?;
    ...
    pub async fn clear_page(&self, page_id: &str) -> Result<()> {
        ...
        loop {
            ...
            if let Some(value) = cursor.as_ref() { url.push_str("&start_cursor="); url.push_str(value); }
            let value = Self::parse(self.request(reqwest::Method::GET, url).send().await?).await?;
            if let Some(results) = value.get("results").and_then(Value::as_array) {
                block_ids.extend(results.iter().filter_map(|block| block.get("id").and_then(Value::as_str)).map(str::to_owned));
            }
        ...
        for block_id in block_ids {
            Self::parse(self.request(reqwest::Method::DELETE, format!("{NOTION_BASE_URL}/blocks/{block_id}")).send().await?).await?;
        }
        Ok(())
    }
    ...
    pub async fn append_blocks(&self, page_id: &str, blocks: Vec<Value>) -> Result<()> {
        ...
        for chunk in blocks.chunks(100) {
            let response = self.request(reqwest::Method::PATCH, format!("{NOTION_BASE_URL}/blocks/{page_id}/children")).json(&json!({ "children": chunk })).send().await?;
            Self::parse(response).await?;
        }
        Ok(())
    }
    ...
    pub async fn upload_file(&self, path: &Path, mime_type: &str) -> Result<String> {
        ...
        if metadata.len() > MAX_SINGLE_PART_FILE_SIZE { anyhow::bail!("文件超过 20 MiB，首版暂不支持分片上传"); }
        let file_name = path.file_name().and_then(|value| value.to_str()).unwrap_or("file");
        let created = Self::parse(self.request(reqwest::Method::POST, format!("{NOTION_BASE_URL}/file_uploads")).json(&json!({
            "mode": "single_part", "filename": file_name, "content_type": mime_type
        })).send().await?).await?;
        let upload_id = created.get("id").and_then(Value::as_str).context("缺少 file_upload id")?;
        let upload_url = created.get("upload_url").and_then(Value::as_str).map(str::to_owned).unwrap_or_else(|| format!("{NOTION_BASE_URL}/file_uploads/{upload_id}/send"));
        let bytes = tokio::fs::read(path).await?;
        let part = multipart::Part::bytes(bytes).file_name(file_name.to_string()).mime_str(mime_type)?;
        let response = self.client.post(upload_url).bearer_auth(&self.token).header("Notion-Version", NOTION_VERSION).multipart(multipart::Form::new().part("file", part)).send().await?;
        let uploaded = Self::parse(response).await?;
        if uploaded.get("status").and_then(Value::as_str) != Some("uploaded") { anyhow::bail!("文件上传后未进入 uploaded 状态"); }
        Ok(upload_id.to_string())
    }
}

pub fn normalize_page_id(value: &str) -> Result<String> {
    let compact: String = value.trim().chars().filter(|character| character.is_ascii_hexdigit()).collect();
    if compact.len() != 32 { anyhow::bail!("页面 ID 格式无效，应为 32 位十六进制 ID"); }
...
pub fn text_blocks(content: &str, extension: &str) -> Vec<Value> {
    if matches!(extension, "md" | "markdown" | "mdx") { markdown_blocks(content) } else { code_blocks(content, notion_code_language(extension)) }
}

pub fn file_block(upload_id: &str, mime_type: &str) -> Value {
    let block_type = if mime_type.starts_with("image/") { "image" } else if mime_type == "application/pdf" { "pdf" } else if mime_type.starts_with("audio/") { "audio" } else if mime_type.starts_with("video/") { "video" } else { "file" };
    let mut block = json!({ "object": "block", "type": block_type });
    block[block_type] = json!({ "type": "file_upload", "file_upload": { "id": upload_id } });
    block
}

pub fn metadata_callout(relative_path: &str, hash: &str) -> Value {
    rich_text_block("callout", &format!("来源：{relative_path}\nSHA-256：{hash}"), Some("📁"))
}
...
fn markdown_blocks(content: &str) -> Vec<Value> {
    ...
    for line in content.lines() {
        if line.trim_start().starts_with("```") {
            if in_code {
                ...
                if language.is_empty() { language = "plain text".to_string(); }
                in_code = true;
            }
            continue;
        }
        if in_code { code.push_str(line); code.push('\n'); continue; }
    ...
    if in_code && !code.is_empty() { blocks.extend(code_block_values(&code, &language)); }
...
fn code_block_values(content: &str, language: &str) -> Vec<Value> {
    split_text(content).into_iter().map(|part| json!({
        "object": "block", "type": "code", "code": { "rich_text": [{ "type": "text", "text": { "content": part } }], "language": language, "caption": [] }
    })).collect()
}
